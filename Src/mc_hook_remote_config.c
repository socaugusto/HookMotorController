...
{
	if(motor_device->status == MC_STOP){                    // Change motor direction when motor stops (!!!the direction should not be changed while motor running).
		MC_Core_SetDirection(motor_device, motor_direction);	
	}
}
...
void hook_motor_control_handle(MC_Handle_t *motor_device, hook_remote_cmd_t *rcmd, STM_Handle_t *stmh)
{
	...
	//Check remote cmd
	if((rcmd->open_pos == 2 && rcmd->mid_pos == 2 && rcmd->close_pos == 0) && (hook_position != HOOK_MECH_HOME_POS_VAL)){ hookProcessStat = HOOK_PROCESS_HOMING; }   
	else if(rcmd->open_pos == 3 && rcmd->mid_pos == 0 && rcmd->close_pos == 3){ hookProcessStat = HOOK_PROCESS_SYSRESET; }
  else if((rcmd->open_pos == 0 && rcmd->mid_pos == 4 && rcmd->close_pos == 4) && ((lastHookStatus == CLOSE_POS) && HOOK_CLOSE_POS_LED_FLAG)){ hookProcessStat = HOOK_PROCESS_TESTING; }
  else {
		//Do nothing
	}		
	
	
	if((rcmd->open_pos == 1 || rcmd->mid_pos == 1 || rcmd->close_pos == 1) && HOOK_HOMING_FLAG) { HOOK_HOMING_MOTOR_STOP_FLAG = true; } 
	...
	switch(hookProcessStat)
	{
	    ...
	    if(rcmd->open_pos == 1 && rcmd->mid_pos == 0 && rcmd->close_pos == 0){ OpenCMDindex_Flag = true; }		
	    else if(rcmd->mid_pos == 1 && rcmd->open_pos == 0 && rcmd->close_pos == 0){ MidCMDindex_Flag = true; }
	    else if(rcmd->close_pos == 1 && rcmd->open_pos == 0 && rcmd->mid_pos == 0){ CloseCMDindex_Flag = true; }
	    else {
				//Do nothing	
	    }
	
	    //Check command index values Determine remote cmd status
	    if(OpenCMDindex_Flag && rcmd->open_pos == 0){ OpenCMDindex++; OpenCMDindex_Flag = false; }
	    if(MidCMDindex_Flag && rcmd->mid_pos == 0){ MidCMDindex++; MidCMDindex_Flag = false; }
	    if(CloseCMDindex_Flag && rcmd->close_pos == 0){ CloseCMDindex++; CloseCMDindex_Flag = false; }
			

			//Determine remote cmd status
      if(OpenCMDindex > 0){ cmd = OPEN_POS_CMD; HOOK_OPEN_POS_CMD_FLAG = true; }
	    if(MidCMDindex > 0){ cmd = MID_POS_CMD; HOOK_MID_POS_CMD_FLAG = true; }
	    if(CloseCMDindex > 0){ cmd = CLOSE_POS_CMD; HOOK_CLOSE_POS_CMD_FLAG = true; }
...
void Position_FeedbackHandle(hook_data_t *hd, MC_Handle_t *motor_device)
{
	...
	if(HOOK_OPEN_POS_CMD_FLAG){
		...
		if(motor_device->status == MC_STOP) hd->open_pos_led = 1;       // 1:led on, 2:led toggle
	}
	if(HOOK_MID_POS_CMD_FLAG){
		hd->mid_pos_led = 2;        // 1:led on, 2:led toggle
		if(motor_device->status == MC_STOP) hd->mid_pos_led = 1;       // 1:led on, 2:led toggle
	}
	if(HOOK_CLOSE_POS_CMD_FLAG){
		hd->close_pos_led = 2;      // 1:led on, 2:led toggle
		if(motor_device->status == MC_STOP) hd->close_pos_led = 1;       // 1:led on, 2:led toggle
	}
	
	//Define fault led condition against unknown position, homing and testing process.
	if(HOOK_UNKNOWN_POS_FLAG){ hd->fault_led = 1; }
	else if(HOOK_HOMING_FLAG){ hd->fault_led = 2; }
	else if(HOOK_TESTING_FLAG){ hd->fault_led = 3; }
	else {
		hd->fault_led = 0;
	}	
}
